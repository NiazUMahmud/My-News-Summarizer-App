#This app does not require any API to run the app, but the user have to use their API to access the app.
...
# --- Helper Functions ---
def setup_newsapi(news_api_key):
    """Initialize NewsAPI client using the provided News API key."""
    if not news_api_key:
...
def summarize_with_openai(text, openai_api_key):
    ...
    try:
        chat = ChatOpenAI(
            temperature=0.5,
            openai_api_key=openai_api_key  # Use the provided API key
        )
...
def main():
    ...
    newsapi_client = setup_newsapi(news_api_key)
    ...
    if st.sidebar.button("🚀 Fetch & Summarize"):
        # Validate API Keys are entered
        if not openai_api_key or not news_api_key:
            st.error("Please enter both OpenAI and NewsAPI API keys.")
            return
                ...
                with col1:
                    if content:
                        ...
                        with st.spinner("Summarizing..."):
                            summary = summarize_with_openai(clean_content, openai_api_key)
